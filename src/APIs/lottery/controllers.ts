...
import { checkNumberService, createLotteryService, getLotteryService, } from "./services";

export const getLotteryController = async (req: Request, res: Response) => {

    let lottery = await getLotteryService()
    if (!lottery) lottery = []
...
export const createLotteryController = async (req: Request, res: Response) => {

    const number = req.body.number;
    const check = await checkNumberService(number)
    if (check) {
        return res.json({
            status: "error",
    ...
    const createUser = await createLotteryService(req.body)
